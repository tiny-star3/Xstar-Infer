...
async def main():
    ...
    # 命中点: 服务端 _tick 里 match_prefix(seq[:-1]) 会命中"上一轮 prompt+generated"种进树的块, 所以 radix on 时第 r>1 轮的 prefill = 只算本轮新增; radix off 时 = 全历史(随轮次线性涨)
    ...
    print(
        f"approx_tokens/turn_prompt = {len(tok(BASE)['input_ids'])} + growing history"
    )
