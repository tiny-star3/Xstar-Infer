...
__global__ void paged_attention_prefill_kernel(T *out, const T *Q, const T *pool,
                                               ...
                                               int num_heads, int num_kv_heads, int block_size,
                                               float scalar)
{
    // 第 seq 个 request 的 query block
    int qb = blockIdx.x;
    // 第 seq 个 request
    int seq = blockIdx.y;
    int head = blockIdx.z;
    ...
    for (int i = threadIdx.x; i < Br * HEAD_DIM; i += blockDim.x)
    {
        ...
        int dim = i % HEAD_DIM;
        // 段内行号
        int local_row_global = qb * Br + local_row;
        if (local_row_global < seq_q)
        {
            // 转全局去索引 Q
            int global_row = cu_seqlens_q[seq] + local_row_global;
            Qs[local_row][dim] = Q[head * cu_seqlens_q[num_seqs] * HEAD_DIM + global_row * HEAD_DIM + dim];
        }
        else
        {
    ...
    for (int i = 0; i < seq_k; i += Bc)
    {
        ...
        for (int j = threadIdx.x; j < Bc * HEAD_DIM; j += blockDim.x)
        {
            ...
            int dim = j % HEAD_DIM;
            // 逻辑 token 号
            int local_key = i + local_row;
            if (local_key < seq_k)
            {
                // 物理块
                int block = d_block_table_2d[seq * max_blocks_per_seq + local_key / block_size];
                int slot = local_key % block_size;
        ...
        int local_query = qb * Br + row;
        int local_key = i + col_offset;
        ...
        if (full_past)
        {
            ...
            {
                if (local_query < seq_q && local_key + t < seq_k)
                {
    ...
    for (int i = 0; i < COL_PER_THREAD; i++)
    {
        ...
        if (gq < seq_q)
        {
            out[(cu_seqlens_q[seq] + gq) * (num_heads * HEAD_DIM) + head * HEAD_DIM + col_offset + i] = (T)(O[i] / l);
        }
    }
}
...
void paged_attention_prefill_launch(void *out, const void *Q, const void *pool,
                                    ...
                                    DType dtype)
{
    // 一个 block 的 Q 行必须同属一个段
    dim3 threadPerBlock(THREADPERBLOCKDIM);
    dim3 blockPerGrid((max_seqlen_q + Br - 1) / Br, num_seqs, num_heads);
    float scalar = 1.0 / sqrt(head_dim);
    if (dtype == DType::Float32)
    {
        if (head_dim == 64)
        {
            paged_attention_prefill_kernel<float, 64><<<blockPerGrid, threadPerBlock>>>(static_cast<float *>(out), static_cast<const float *>(Q), static_cast<const float *>(pool), d_block_table_2d, cu_seqlens_q, cu_seqlens_k, num_seqs, max_blocks_per_seq, layer_stride_elems, block_elems, layer, num_heads, num_kv_heads, block_size, scalar);
    ...
    else if (dtype == DType::BFloat16)
    {
        if (head_dim == 64)
        {
            paged_attention_prefill_kernel<__nv_bfloat16, 64><<<blockPerGrid, threadPerBlock>>>(static_cast<__nv_bfloat16 *>(out), static_cast<const __nv_bfloat16 *>(Q), static_cast<const __nv_bfloat16 *>(pool), d_block_table_2d, cu_seqlens_q, cu_seqlens_k, num_seqs, max_blocks_per_seq, layer_stride_elems, block_elems, layer, num_heads, num_kv_heads, block_size, scalar);
            CHECK_CUDA(cudaGetLastError());
            CHECK_CUDA(cudaDeviceSynchronize());
        }
        else if (head_dim == 128)
