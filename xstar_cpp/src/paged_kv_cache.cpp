...
PagedKVCache::PagedKVCache(std::int64_t num_kv_heads, std::int64_t head_dim, std::int64_t max_seq_len, std::int64_t block_size, DType dtype, Device device) : num_kv_heads_(num_kv_heads), head_dim_(head_dim), max_seq_len_(max_seq_len), block_size_(block_size), dtype_(dtype), device_(device), d_block_table_(nullptr)
{
    cursor_ = 0;
...
void PagedKVCache::write(std::int64_t layer, BlockManager &bm, const Tensor &K, const Tensor &V, bool is_decode)
{
    ...
    if (device_ == Device::CUDA)
    {
        ...
        else
        {
            ...
            {
                if (cursor_ != 0)
                    throw std::runtime_error("prefill, but cursor != 0");
            ...
            {
                slot_mapping[i] = block_table_[i / block_size_] * block_size_ + i % block_size_;
            }
